...
class StoreDroidandiSpider(scrapy.Spider):
    name = "droidandi"
    base_url = furl("https://droidandi.com/")
    start_urls = [f"https://droidandi.com/collection/all"]
    ...
    def parse(self, response, **kwargs):
        ...
        if maximum_parsing_pages is not None:
            ...
            max_page_number = int(response.css(".pagination-items a.button::text").getall()[-1].strip())
    ...
    def _parse_catalog_page(self, response, **kwargs):
        for item_url_path in response.css(".product-preview__title a").xpath("@href").getall():
    ...
    def _parse_item_page(self, response):
        item_image = response.css(".product__photo").xpath("@href").get()
