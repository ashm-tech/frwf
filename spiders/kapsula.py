...
class StoreKapsulaSpider(scrapy.Spider):
    name = "kapsula"
    base_url = furl("https://kapsula.shop/")
    start_urls = ["https://kapsula.shop/odezhda-i-obuv/"]
    ...
    def parse(self, response, **kwargs):
        ...
        if maximum_parsing_pages is not None:
            ...
            max_page_number = int(response.css(".page-link").xpath("@href")[-1].get().split("?page=")[-1])
    ...
    def _parse_catalog_page(self, response, **kwargs):
        for item_url_path in response.css(".itemname").xpath("@href").getall():
    ...
    def _parse_item_page(self, response):
        ...
        item_name = response.css(".productinfo h1::text").get()
        item_price = int(response.css(".itemprice span span::text").get().replace(" ", ""))
