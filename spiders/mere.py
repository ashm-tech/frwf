...
class StoreMereSpider(scrapy.Spider):
    name = "mere"
    base_url = furl("https://mere.moscow/")
    start_urls = ["https://mere.moscow/collection/all"]
    ...
    def parse(self, response, **kwargs):
        ...
        if maximum_parsing_pages is not None:
            ...
            max_page_number = int(response.css(".pagination-link::text")[-3].get().strip())
    ...
    def _parse_catalog_page(self, response, **kwargs):
        for item_url_path in response.css(".cell-3 .product-image").xpath("@href").getall():
