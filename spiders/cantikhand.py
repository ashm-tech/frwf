...
class StoreCantikhandSpider(scrapy.Spider):
    name = "cantikhand"
    ...
    def _parse_item_page(self, response):
        ...
        item_name = response.css('meta[property="og:description"]').xpath("@content").get()
        item_price = int(re.sub(r"[^0-9]", "", response.css('meta[itemprop="price"]').xpath("@content").get()))
